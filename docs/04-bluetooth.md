...
## Purpose 🎯
- Quickly enable Bluetooth and run a short scan (about 10 seconds) to show discovered devices ⏱️.
- The helper script restarts audio services when needed 🔄 and applies simple filtering to hide other computers from the results.

Important: the script can enable Bluetooth but it does not automatically disable Airplane Mode ✈️ — if Airplane Mode is on, turn it off manually before running the script.
...
- It filters the device list to hide unrelated or other computers 🚫🖥️.
